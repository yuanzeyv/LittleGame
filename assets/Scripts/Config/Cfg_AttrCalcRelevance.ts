export interface IAttrCalcRelevanceStruct{
	...
	name:string; //属性名称
	fixed:number; //固定属性
	fixedPercent:number; //固定属性加成
	sumPercent:number; //总属性加成
	isPercent:boolean; //属性是否万分比
};
class AttrCalcRelevance{  
    private mConfigObject:{[key:number]: IAttrCalcRelevanceStruct}  = {};
    private mConfigArray:Array<IAttrCalcRelevanceStruct> = new Array<IAttrCalcRelevanceStruct>();
    constructor(){
        this.InitConfig();
        this.InitArray();
    }
...
export let AttrCalcRelevanceConfig:AttrCalcRelevance = new AttrCalcRelevance();
