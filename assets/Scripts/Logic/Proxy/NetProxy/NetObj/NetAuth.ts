...
export class NetAuth extends NetCellBase{
    ...
    Init(): void {
        _Facade.Send(eNotificationEnum.NetStartConnect);//网络开始连接
    }
    OpenHandle(ev: Event): void {
        this.m_ConnectSuccessFlag = true;//代表当前连接成功了
        _Facade.Send(eNotificationEnum.NetConnectSuccess);//网络连接成功
        setTimeout(()=>{
            _Facade.Send(eNotificationEnum.NetStartAuth);//开始验证用户名或密码
    ...
    CloseHandle(ev: CloseEvent): void {
        ...
        if(!this.m_ConnectSuccessFlag){
            _Facade.Send(eNotificationEnum.NetConnectFail);//网络连接失败
            netProxy.ClearNetCell();
            return;
        }
        _Facade.Send(eNotificationEnum.NetAuthFail);//有可能验证超时了
    ...
    MessageHandle(ev: MessageEvent<any>): void {
        ...
        if(auth.Error){
            _Facade.Send(eNotificationEnum.TipsShow,auth.ErrorData);
            return;
        }
        if(auth.Status == true){//成功的话
            netProxy.SetSocketCell(new NetConnect(this.m_WebSocket));
            if(auth.IsReConnect){
                _Facade.Send(eNotificationEnum.M_NetReconnectSuccess);
            }
            else{
                _Facade.Send(eNotificationEnum.NetAuthSuccess);//发送验证成功的消息 
            }
        }
        else {
            if(auth.IsReConnect)
                _Facade.Send(eNotificationEnum.M_NetReconnectFail);
            else
                _Facade.Send(eNotificationEnum.NetAuthFail);
