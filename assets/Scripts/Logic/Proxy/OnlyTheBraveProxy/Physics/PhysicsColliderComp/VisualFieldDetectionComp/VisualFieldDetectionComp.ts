...
export abstract class VisualFieldDetectionComp extends PhysicsCompBase{  
    public OnInit(){ 
        this.mCollider.SetColliderActiveEvent(true,false);
        this.mCollider.Sensor = true;
        this.mCollider.SetCollidersGroup(this.ColliderGroups);
    }   

    protected abstract get ColliderGroups():number;
    protected abstract get ColliderDetectionType():eVisualFieldDetectionType;
    
    //开始碰撞回调   
    public OnStartConcatCollider(playerBase:ColliderBase):void{
        this.Player.DetectionPlayerBaseEnter(playerBase.Parent.RelevanceObj as PlayerBase,this.ColliderDetectionType);
    } 
    
    //结束碰撞回调   
    public OnLeaveConcatCollider(playerBase:ColliderBase):void{
        this.Player.DetectionPlayerBaseLeave(playerBase.Parent.RelevanceObj as PlayerBase,this.ColliderDetectionType);
    } 
} 