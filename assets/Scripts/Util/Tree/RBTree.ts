...
export class RBNode<K, V>{//红黑树对象
    public Parent: RBNode<K, V>;//父亲节点 
    public Left: RBNode<K, V>;//左节点
    public Right: RBNode<K, V>;//右节点
    public Key: K;//当前的键值 
    public Value: V;//当前的值  
...
function RreeCompare(keyA,keyB):number{
    if(keyA == keyB)
        return 0;
...
export class RBTree<K, V>{
    ...
    constructor(copareHandle: CompareDivisor<K> = RreeCompare) {
    ...
    public Set(key: K, value?: V): RBNode<K, V> {
        ...
        return this.Insert(key, value);//否则插入
    }
